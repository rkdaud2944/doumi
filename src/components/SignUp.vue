          ...
          <div v-if="checkedValue === 'one'">
            <form style="margin-left: 1%; margin-right: auto;">
              ...
              <div class="modal-footer">
                  <button type="submit" v-if="checkedValue === 'one'" @click="onedayclick">신청</button>
              </div>
            </form>
          </div>
      ...
      methods: {
          ...
          onedayclick() {
              if (this.emailclear === true){
                  axios
                  .post(
                      `http://localhost:8081/reserve/api/v1/admin/addReservation?teacherName=${this.teacherName}&phone=${this.phoneNumber}&email=${this.email}&studentsPerGroup=${this.studentCount}&curriculumSn=${this.curriculumSn}&schoolSn=1&classDate=${this.selectedDate}&classDate2=${this.classDate}&classSchedule=${this.selectedTimes.join(',')}`
                  )
                  .then((response) => {
                      console.log('aaa :' + response.data)
